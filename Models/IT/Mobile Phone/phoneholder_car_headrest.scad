...
// Models/IT/Mobile Phone/phoneholder_car_headrest.scad (https://github.com/RichardBernards/openscad/)
//
...
phoneholder_car_headrest();


module phoneholder_car_headrest() {
  ...
  module _frame() {
    ...
    union() {
      ...
      rotate([0,0,strutRotation])_frameStrut();
      rotate([0,0,-strutRotation])_frameStrut();
    }
  }
  module _headrestGrip() {
  ...
  module _headrestMount() {
    braceWidth = (rodDistance-rodDiameter-rodClearance);
    union() {
      translate([-(0.5*rodDistance),0,0]) _headrestGrip();
      translate([(0.5*rodDistance),0,0]) mirror([1,0,0]) _headrestGrip();
      translate([-(0.5*braceWidth),0,0]) cube([braceWidth, wall, frameHeight]);
    }
  }

  union() {
    translate([0,(0.5*frameDepth),0]) _headrestMount();
    _frame();
    translate([0,-(0.5*frameDepth),0]) _phoneHolder();
  }
}
