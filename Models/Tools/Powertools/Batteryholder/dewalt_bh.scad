...
// Models/Tools/Powertools/Batteryholder/dewalt_bh.scad (https://github.com/RichardBernards/openscad/)
//
...
/* [DeWalt Battery Holder] */
// Select holder type
batteryHolderType = 1; // [0:Regular, 1:Minimal, 2:Minimal with lock]
    ...
    if(batteryCount > 1) {
      ...
      union() {
        for(batCount = [0:1:(batteryCount-1)]) {
          translate([0,(batCount*(dims[1]+batDistance)),0]) dewalt_bh(batteryHolderType);
          if(batCount > 0) {
      ...
      dewalt_bh(batteryHolderType);
    }

    if(mountingType == 1) { // screwholes
      for(screwLine = [0:1:(batteryCount-1)]) {
        translate([((0.5*dims[0])-(3*screwDiameter)),((0.5*dims[1])+(screwLine*usedBatteryWidth)),floorHeight]) screwcutout(dims[1]);
        translate([((0.5*dims[0])+(3*screwDiameter)),((0.5*dims[1])+(screwLine*usedBatteryWidth)),floorHeight]) screwcutout(dims[1]);
      }
    }
      ...
      for(skadisYpos = [0:1:(countSkadisMounts-1)]) {
        ...
        for(check = _grBounds) {
          if( (check[0] < skBounds[0]) && (check[1] > skBounds[1]) ) {
            ...
            translate([xPos,translationPos,-0.01]) rotate([-90,0,-90]) tclipcutout(50);
          }
        }
      }
    }
...
/*
//Following kept in for reference purposes....


module batteryHolder() {
  union() {
    linear_extrude(18.6) difference() {
      union() {
        translate([0,10,0]) resize([26,40,0]) circle(d=26);
        translate([0,-10,0]) resize([26,40,0]) circle(d=26);
        translate([-13,-10,0]) square([26,20]);
        translate([0,-30,0]) square([(71.8-13), 60]);
      }
      translate([(71.8-13-60),-26,0]) square([60.1,52]);
    }
    linear_extrude(7.6) difference() {
      polygon([
        [(71.8-13-60), 26],
        [(71.8-13-60), -26],
        [(71.8-13), -26],
      ...
      translate([(71.8-13-7),-13,0]) square([7.1,26]);
    }
    translate([(71.8-13-60),21,(18.6-2.2)]) rotate([90,0,90]) linear_extrude(42) polygon([ [0,0],[5,0],[5,-5] ]);
    translate([(71.8-13-60),-21,(18.6-2.2)]) rotate([90,0,90]) linear_extrude(42) polygon([ [0,0],[-5,0],[-5,-5] ]);

    translate([(71.8-13),30,0]) rotate([90,0,0]) linear_extrude(60) polygon([
      [0,0],
      [15.834,0],
      [16.534,0.7],
      [15.273,2.5],
    ...
    translate([(71.8-13),30,0]) rotate([90,0,0]) linear_extrude(4) union() {
      intersection() {
        translate([-0.1,10.6,0]) difference() {
          circle(r=8);
          translate([-10,-8.5,0]) square([10,17]);
          translate([-8.5,-10,0]) square([17,10]);
        }
        polygon([ [0,7.6],[0,18.6],[4,18.6],[15.273,2.5],[9.1,2.5],[1,7.6] ]);
      }
      difference() {
        polygon([ [0,7.6],[0,18.6],[4,18.6],[15.273,2.5],[9.1,2.5],[1,7.6] ]);
        translate([-0.1,15,0]) square([16,10]);
      }
    }
    translate([(71.8-13),-26,0]) rotate([90,0,0]) linear_extrude(4) union() {
      intersection() {
        translate([-0.1,10.6,0]) difference() {
          circle(r=8);
          translate([-10,-8.5,0]) square([10,17]);
